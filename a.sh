...
cat << 'EOF' > src/components/AppHeader.vue
<template>
  <v-app-bar color="primary" prominent>
    <v-avatar class="ml-4">
      <v-img src="https://picsum.photos/seed/logo/100/100" alt="Logo"></v-img>
    </v-avatar>
    <v-app-bar-title>Mi Portafolio Fotográfico</v-app-bar-title>
  </v-app-bar>
</template>
...
cat << 'EOF' > src/components/AppFooter.vue
<template>
    ...
    <v-container>
      <v-row>
        ...
        <v-col cols="12" md="4" class="text-center">
          <span>Matrícula: 12345678</span>
        </v-col>
        <v-col cols="12" md="4" class="text-center text-md-right">
          <span>&copy; {{ currentYear }} - contacto@universidad.edu</span>
        </v-col>
      </v-row>
    </v-container>
  </v-footer>
...
cat << 'EOF' > src/components/TablaDeDatos.vue
<template>
  <v-table>
    <thead>
      <tr>
        <th class="text-left">Habilidad Técnica</th>
        <th class="text-left">Nivel de Dominio</th>
        <th class="text-left">Años de Experiencia</th>
      </tr>
    </thead>
    <tbody>
      <tr v-for="item in habilidades" :key="item.nombre">
        <td>{{ item.nombre }}</td>
        <td>{{ item.nivel }}</td>
        <td>{{ item.anios }}</td>
      </tr>
...
const habilidades = ref([
  { nombre: 'Vue.js 3', nivel: 'Intermedio', anios: 1 },
  { nombre: 'JavaScript (ES6+)', nivel: 'Avanzado', anios: 2 },
  { nombre: 'Vuetify 3', nivel: 'Básico', anios: 1 },
  { nombre: 'Git & GitHub', nivel: 'Intermedio', anios: 2 }
])
</script>
EOF

cat << 'EOF' > src/components/TarjetaConImagen.vue
<template>
  <v-card class="mx-auto" max-width="400">
    <v-img :src="url" height="200px" cover></v-img>
    <v-card-title>{{ titulo }}</v-card-title>
    <v-card-subtitle>Autor: {{ autor }}</v-card-subtitle>
